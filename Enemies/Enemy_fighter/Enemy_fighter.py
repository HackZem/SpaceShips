...
from Enemies.Enemy_fighter import Bullet_fighter


class Enemy_fighter(pygame.sprite.Sprite):
    def __init__(self, img, screen, speed, health, x, y, time_move, time_shoot, player, bullets, money):
        super(Enemy_fighter, self).__init__()
        self.screen = screen
    ...
    def shoot(self):
        if self.time_shoot <= 0:
            self.bullets.add(
                Bullet_fighter.BulletEnemy("IMG/bullet_one.png", self.screen, self.player, 9.6, 30, self.rect.centerx + 5,
                                           self.rect.bottom))
            self.bullets.add(
                Bullet_fighter.BulletEnemy("IMG/bullet_one.png", self.screen, self.player, 9.6, 30, self.rect.centerx - 5,
                                           self.rect.bottom))
